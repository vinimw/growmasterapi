...
def build_request_params_post(endpoint):
    base_url = config.endpoints[endpoint]
    url = base_url
    headers = {"x-client-id": config.clientId}
    return url, headers
...
def build_request_params_patch(endpoint, id_update):
    base_url = config.endpoints[endpoint]
    url = f"{base_url}/{id_update}"
    headers = {"x-client-id": config.clientId}
    return url, headers
   
async def patch_external_data(url, data, headers=None):
    ssl_context = ssl.create_default_context()
    ssl_context.check_hostname = False
    ssl_context.verify_mode = ssl.CERT_NONE
...
def build_request_params_delete(endpoint, id_delete):
    base_url = config.endpoints[endpoint]
    url = f"{base_url}/{id_delete}"
    headers = {"x-client-id": config.clientId}
    return url, headers

async def delete_external_data(url, headers=None):
    ssl_context = ssl.create_default_context()
    ssl_context.check_hostname = False
    ssl_context.verify_mode = ssl.CERT_NONE
